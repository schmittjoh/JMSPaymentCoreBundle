        ...
        foreach ($container->findTaggedServiceIds('payment.method_form_type') as $id => $attrs) {
            ...
            if (!$attrs) {
                throw new RuntimeException(sprintf(
                    'The service "%s" is marked as payment method form type (tagged with "payment.method_form_type"), but is not registered as a form type with the Form Component. Please also add a "form.type" tag.',
                    $id
                ));
            }
            ...
            if (!isset($attrs[0]['alias'])) {
                throw new RuntimeException(sprintf(
                    'Please define an alias attribute for tag "form.type" of service "%s".',
                    $id
                ));
            }
