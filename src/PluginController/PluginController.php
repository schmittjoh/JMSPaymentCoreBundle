...
abstract class PluginController implements PluginControllerInterface
{
     ...
     * {@inheritdoc}
     */
    public function checkPaymentInstruction(PaymentInstructionInterface $paymentInstruction): Result
    {
        $plugin = $this->getPlugin($paymentInstruction->getPaymentSystemName());

        try {
            $plugin->checkPaymentInstruction($paymentInstruction);
            ...
            return $this->onUnsuccessfulPaymentInstructionValidation($paymentInstruction, $invalidInstruction);
        }
    }
     ...
     * {@inheritdoc}
     */
    public function closePaymentInstruction(PaymentInstructionInterface $paymentInstruction): void
    {
        $oldState = $paymentInstruction->getState();

        $paymentInstruction->setState(PaymentInstructionInterface::STATE_CLOSED);

        $this->dispatchPaymentInstructionStateChange($paymentInstruction, $oldState);
    }

    /**
     * {@inheritdoc}
     */
    public function createPayment($paymentInstructionId, $amount): PaymentInterface
    {
     ...
     * {@inheritdoc}
     */
    public function createPaymentInstruction(PaymentInstructionInterface $paymentInstruction): void
    {
    ...
    public function getPaymentInstruction($paymentInstructionId, $maskSensitiveData = true): PaymentInstructionInterface
    {
        $paymentInstruction = $this->doGetPaymentInstruction($paymentInstructionId);
     ...
     * {@inheritdoc}
     */
    public function getRemainingValueOnPaymentInstruction(PaymentInstructionInterface $paymentInstruction): ?float
    {
        $plugin = $this->getPlugin($paymentInstruction->getPaymentSystemName());
        ...
        return $plugin->getAvailableBalance($paymentInstruction);
    }

    /**
     * {@inheritdoc}
     */
    public function validatePaymentInstruction(PaymentInstructionInterface $paymentInstruction): Result
    {
        $plugin = $this->getPlugin($paymentInstruction->getPaymentSystemName());
    ...
    protected function doApprove(PaymentInterface $payment, $amount)
    {
        ...
        if (PaymentInterface::STATE_NEW === $paymentState) {
            ...
            if ($instruction->hasPendingTransaction()) {
                throw new InvalidPaymentInstructionException(
                    'The PaymentInstruction can only ever have one pending transaction.'
                );
            }
            ...
            if (0 !== Number::compare($payment->getTargetAmount(), $amount)) {
                throw new Exception(
                    'The Payment\'s target amount must equal the requested amount in a retry transaction.'
                );
            }
        ...
        try {
            ...
            if (PluginInterface::RESPONSE_CODE_SUCCESS === $transaction->getResponseCode()) {
                ...
                return $this->buildFinancialTransactionResult(
                    $transaction,
                    Result::STATUS_SUCCESS,
                    PluginInterface::REASON_CODE_SUCCESS
                );
            } else {
            ...
            $result = $this->buildFinancialTransactionResult(
                $transaction,
                Result::STATUS_FAILED,
                $transaction->getReasonCode()
            );
    ...
    protected function doApproveAndDeposit(PaymentInterface $payment, $amount)
    {
        ...
        if (PaymentInterface::STATE_NEW === $paymentState) {
            if ($instruction->hasPendingTransaction()) {
                throw new InvalidPaymentInstructionException(
                    'PaymentInstruction can only ever have one pending transaction.'
                );
            }
    ...
    protected function doCredit(CreditInterface $credit, $amount)
    {
        ...
        if (CreditInterface::STATE_NEW === $creditState) {
            if (1 === Number::compare(
                $amount,
                $max = $instruction->getDepositedAmount() - $instruction->getReversingDepositedAmount(
                ) - $instruction->getCreditingAmount() - $instruction->getCreditedAmount()
            )) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (PaymentInstruction restriction).', $max)
                );
            }

            if (1 === Number::compare($amount, $credit->getTargetAmount())) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (Credit restriction).', $credit->getTargetAmount())
                );
            }
            ...
            if (false === $credit->isIndependent()) {
                ...
                if (1 === Number::compare(
                    $amount,
                    $max = $payment->getDepositedAmount() - $payment->getReversingDepositedAmount(
                    ) - $payment->getCreditingAmount() - $payment->getCreditedAmount()
                )) {
                    throw new InvalidArgumentException(
                        sprintf('$amount cannot be greater than %.2f (Payment restriction).', $max)
                    );
                }
            }
            ...
            if (1 === Number::compare($amount, $instruction->getCreditingAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount cannot be greater than %.2f (PaymentInstruction restriction).',
                        $instruction->getCreditingAmount()
                    )
                );
            }
            if (0 !== Number::compare($amount, $credit->getCreditingAmount())) {
                throw new InvalidArgumentException(
                    sprintf('$amount must be equal to %.2f (Credit restriction).', $credit->getCreditingAmount())
                );
            }
            ...
            if (false === $credit->isIndependent()) {
                ...
                if (1 === Number::compare($amount, $payment->getCreditingAmount())) {
                    throw new InvalidArgumentException(
                        sprintf(
                            '$amount cannot be greater than %.2f (Payment restriction).',
                            $payment->getCreditingAmount()
                        )
                    );
                }
            }
    ...
    protected function doDeposit(PaymentInterface $payment, $amount)
    {
        ...
        if (PaymentInterface::STATE_APPROVED === $paymentState) {
            if ($instruction->hasPendingTransaction()) {
                throw new InvalidPaymentInstructionException(
                    'The PaymentInstruction can only have one pending transaction at a time.'
                );
            }
            ...
            if (null === $transaction) {
                if (1 === Number::compare($amount, $payment->getApprovedAmount() - $payment->getDepositedAmount())) {
                    throw new Exception(
                        'The amount cannot be greater than the approved amount minus the already deposited amount.'
                    );
                }
                ...
                if ($transaction->getPayment()->getId() !== $payment->getId()) {
                    throw new InvalidPaymentInstructionException(
                        'The PaymentInstruction has a pending transaction on another Payment.'
                    );
                }

                if (0 !== Number::compare($transaction->getRequestedAmount(), $amount)) {
                    throw new Exception(
                        'The requested amount must be equal to the transaction\'s amount when retrying.'
                    );
                }
        ...
        try {
            ...
            if (PluginInterface::RESPONSE_CODE_SUCCESS === $transaction->getResponseCode()) {
                $payment->setDepositingAmount(0.0);
                $payment->setDepositedAmount(
                    $depositedAmount = $payment->getDepositedAmount() + $transaction->getProcessedAmount()
                );
                ...
                $instruction->setDepositingAmount($instruction->getDepositingAmount() - $amount);
                $instruction->setDepositedAmount(
                    $instruction->getDepositedAmount() + $transaction->getProcessedAmount()
                );
    ...
    protected function doReverseApproval(PaymentInterface $payment, $amount)
    {
        ...
        if (null === $transaction) {
            if (1 === Number::compare(
                $amount,
                $max = $instruction->getApprovedAmount() - $instruction->getReversingApprovedAmount()
            )) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (PaymentInstruction restriction).', $max)
                );
            ...
            if (1 === Number::compare($amount, $payment->getApprovedAmount())) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (Payment restriction).', $payment->getApprovedAmount())
                );
            }
            ...
            if (1 === Number::compare($amount, $instruction->getReversingApprovedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount cannot be greater than %.2f (PaymentInstruction restriction).',
                        $instruction->getReversingApprovedAmount()
                    )
                );
            }
            ...
            if (0 !== Number::compare($amount, $payment->getReversingApprovedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount must be equal to %.2f (Payment restriction).',
                        $payment->getReversingApprovedAmount()
                    )
                );
            }
    ...
    protected function doReverseCredit(CreditInterface $credit, $amount)
    {
        ...
        if (false === $credit->isIndependent()) {
            if (PaymentInterface::STATE_APPROVED !== $payment->getState(
            ) && PaymentInterface::STATE_EXPIRED !== $payment->getState()) {
        ...
        if (null === $transaction) {
            if (1 === Number::compare(
                $amount,
                $max = $instruction->getCreditedAmount() - $instruction->getReversingCreditedAmount()
            )) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (PaymentInstruction restriction).', $max)
                );
            ...
            if (1 === Number::compare($amount, $credit->getCreditedAmount())) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (Credit restriction).', $credit->getCreditedAmount())
                );
            }

            if (false === $credit->isIndependent() && 1 === Number::compare(
                $amount,
                $max = $payment->getCreditedAmount() - $payment->getReversingCreditedAmount()
            )) {
                throw new InvalidArgumentException(
                    sprintf('$amount cannot be greater than %.2f (Payment restriction).', $max)
                );
            ...
            if (1 === Number::compare($amount, $instruction->getReversingCreditedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount cannot be greater than %.2f (PaymentInstruction restriction).',
                        $instruction->getReversingCreditedAmount()
                    )
                );
            }
            ...
            if (0 !== Number::compare($amount, $credit->getReversingCreditedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount must be equal to %.2f (Credit restriction).',
                        $credit->getReversingCreditedAmount()
                    )
                );
            }
            ...
            if (false === $credit->isIndependent() && 1 === Number::compare(
                $amount,
                $payment->getReversingCreditedAmount()
            )) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount cannot be greater than %.2f (Payment restriction).',
        ...
        if (null === $transaction) {
            ...
            if (1 === Number::compare($amount, $instruction->getReversingDepositedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount cannot be greater than %.2f (PaymentInstruction restriction).',
                        $instruction->getReversingDepositedAmount()
                    )
                );
            }
            ...
            if (0 !== Number::compare($amount, $payment->getReversingDepositedAmount())) {
                throw new InvalidArgumentException(
                    sprintf(
                        '$amount must be equal to %.2f (Payment restriction).',
                        $payment->getReversingDepositedAmount()
                    )
                );
            }
    ...
    protected function getPlugin($paymentSystemName)
    {
        ...
        throw new PluginNotFoundException(
            sprintf('There is no plugin that processes payments for "%s".', $paymentSystemName)
        );
    }
    ...
    protected function onSuccessfulPaymentInstructionValidation(PaymentInstructionInterface $instruction)
    {
        ...
        return $this->buildPaymentInstructionResult(
            $instruction,
            Result::STATUS_SUCCESS,
            PluginInterface::REASON_CODE_SUCCESS
        );
    ...
    protected function onUnsuccessfulPaymentInstructionValidation(
        PaymentInstructionInterface $instruction,
        PluginInvalidPaymentInstructionException $invalid
    ) {
        $oldState = $instruction->getState();
        ...
        $result = $this->buildPaymentInstructionResult(
            $instruction,
            Result::STATUS_FAILED,
            PluginInterface::REASON_CODE_INVALID
        );
    ...
    private function dispatchPaymentInstructionStateChange(
        PaymentInstructionInterface $instruction,
        int $oldState
    ): void {
    ...
    private function dispatchPaymentStateChange(PaymentInterface $payment, int $oldState): void
    {
        if (null === $this->dispatcher) {
            return;
        }
