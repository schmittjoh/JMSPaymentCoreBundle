...
abstract class GatewayPlugin extends AbstractPlugin
{
    ...
    public function request(Request $request): Response
    {
        ...
        if ('POST' === $method) {
            ...
            if (!$request->headers->has('Content-Type') || 'multipart/form-data' !== $request->headers->get(
                'Content-Type'
            )) {
        ...
        if (false === $returnTransfer = curl_exec($curl)) {
            throw new CommunicationException('cURL Error: ' . curl_error($curl), curl_errno($curl));
        }
